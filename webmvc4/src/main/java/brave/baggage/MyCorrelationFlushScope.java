...
/** Sets up thread locals needed to support {@link MyCorrelationScopeConfig#flushOnUpdate()} */
final class  MyCorrelationFlushScope extends AtomicBoolean implements MyCurrentTraceContext.Scope {
    final  MyCorrelationUpdateScope updateScope;

     MyCorrelationFlushScope( MyCorrelationUpdateScope updateScope) {
     ...
     * MyBaggageField#updateValue(String)}.
     */
    static void flush(MyBaggageField field, String value) {
        Set<MyCorrelationContext> syncedContexts = new LinkedHashSet<>();
        for (Object o : updateScopeStack()) {
             MyCorrelationUpdateScope next = (( MyCorrelationUpdateScope) o);
    ...
    static void pushCurrentUpdateScope( MyCorrelationUpdateScope updateScope) {
        updateScopeStack().push(updateScope);
    }

    static void popCurrentUpdateScope( MyCorrelationUpdateScope expected) {
