...
import brave.propagation.MyCurrentTraceContext.Scope;
...
public class MyTracer {
    ...
    final MyCurrentTraceContext currentTraceContext;
    ...
    MyTracer(
            ...
            Sampler sampler,
            MyCurrentTraceContext currentTraceContext,
     ...
     */
    @Deprecated public MyTracer withSampler(Sampler sampler) {
     ...
     * fork a new child via {@link #newChild(MyTraceContext)}.
     *
     * @see Propagation
     ...
     */
    public final Span joinSpan(MyTraceContext context) {
    ...
    MyTraceContext swapForPendingContext(MyTraceContext context) {
     ...
     * #nextSpan()}.
     */
    public Span newChild(MyTraceContext parent) {
        if (parent == null) throw new NullPointerException("parent == null");
        return _toSpan(parent, decorateContext(parent, parent.spanId()));
    }

    MyTraceContext newRootContext(int flags) {
     ...
     * #newChild(MyTraceContext)} and {@link #startScopedSpanWithParent(String, TraceContext)}. This
     * implies the {@link TraceContext#localRootId()} could be zero, if the context was manually
     * created.
     */
    MyTraceContext decorateContext(MyTraceContext parent, long parentId) {
     ...
     */
    MyTraceContext decorateContext(
            int flags,
            long traceIdHigh,
            long traceId,
            long localRootId,
        ...
        return propagationFactory.decorate(MyInternalPropagation.instance.newTraceContext(
                flags,
                traceIdHigh,
                traceId,
                localRootId,
     ...
     * }</pre>
     *
     * <p><em>Note:</em> Unlike {@link #joinSpan(MyTraceContext)}, this does not attempt to re-use
    ...
    public Span nextSpan(TraceContextOrSamplingFlags extracted) {
        ...
        MyTraceContext context = extracted.context();
        ...
        MyTraceContext parent = currentTraceContext.get();
        int flags;
     ...
     */
    public Span toSpan(MyTraceContext context) {
        return toSpan(null, context);
    }

    Span toSpan(@Nullable MyTraceContext parent, MyTraceContext context) {
        // Re-use a pending context if present: This ensures reference consistency on Span.context()
        MyTraceContext pendingContext = swapForPendingContext(context);
        ...
        MyTraceContext decorated = decorateContext(
    ...
    Span _toSpan(@Nullable MyTraceContext parent, MyTraceContext context) {
        ...
        MyTraceContext pendingContext = pendingSpan.context();
        // A lost race of MyTracer.toSpan(context) is the only known situation where "context" won't be
    ...
    public SpanCustomizer currentSpanCustomizer() {
        ...
        MyTraceContext context = currentTraceContext.get();
     ...
     * @param samplerFunction invoked if there's no {@link MyCurrentTraceContext#get() current trace}
    ...
    public <T> Span nextSpanWithParent(SamplerFunction<T> samplerFunction, T arg,
                                       @Nullable MyTraceContext parent) {
        return _toSpan(parent, nextContext(samplerFunction, arg, parent));
    }

    <T> MyTraceContext nextContext(SamplerFunction<T> samplerFunction, T arg, MyTraceContext parent) {
    ...
    public ScopedSpan startScopedSpanWithParent(String name, @Nullable MyTraceContext parent) {
        if (name == null) throw new NullPointerException("name == null");
    ...
    ScopedSpan newScopedSpan(@Nullable MyTraceContext parent, MyTraceContext context, String name) {
    ...
    @Override public String toString() {
        MyTraceContext currentSpan = currentTraceContext.get();
        return "MyTracer{"
    ...
    boolean isNoop(MyTraceContext context) {
