 ...
 */
public final class MyB3SingleFormat {
     ...
     * use {@link #writeB3SingleFormat(MyTraceContext)} instead, as RPC servers often share a span ID
     * with the client.
     */
    public static String writeB3SingleFormatWithoutParentId(MyTraceContext context) {
    ...
    /**
     * Like {@link #writeB3SingleFormatWithoutParentId(MyTraceContext)}, but for requests with byte
     * array or byte buffer values. For example, {@link ByteBuffer#wrap(byte[])} can wrap the result.
     */
    public static byte[] writeB3SingleFormatWithoutParentIdAsBytes(MyTraceContext context) {
        char[] buffer = RecyclableBuffers.parseBuffer();
        int length = writeB3SingleFormat(context, 0L, buffer);
     ...
     * reuses a client's span ID, prefer {@link #writeB3SingleFormatWithoutParentId(MyTraceContext)}.
     */
    public static String writeB3SingleFormat(MyTraceContext context) {
        char[] buffer = RecyclableBuffers.parseBuffer();
    ...
    /**
     * Like {@link #writeB3SingleFormat(MyTraceContext)}, but for requests with byte array or byte
     * buffer values. For example, {@link ByteBuffer#wrap(byte[])} can wrap the result.
     */
    public static byte[] writeB3SingleFormatAsBytes(MyTraceContext context) {
        char[] buffer = RecyclableBuffers.parseBuffer();
        int length = writeB3SingleFormat(context, context.parentIdAsLong(), buffer);
        return asciiToNewByteArray(buffer, length);
    }

    static int writeB3SingleFormat(MyTraceContext context, long parentId, char[] result) {
        int pos = 0;
    ...
    @Nullable
    public static MyTraceContextOrSamplingFlags parseB3SingleFormat(CharSequence b3) {
     ...
     */
    @Nullable
    public static MyTraceContextOrSamplingFlags parseB3SingleFormat(CharSequence value, int beginIndex,
                                                                  int endIndex) {
        ...
        if (length == 0) {
            ...
            MySamplingFlags flags = tryParseSamplingFlags(value.charAt(beginIndex));
            return flags != null ? MyTraceContextOrSamplingFlags.create(flags) : null;
            ...
            if (c == '-') {
                ...
                switch (currentField) {
                    ...
                    case FIELD_SPAN_ID:
                        ...
                        break;
                    case FIELD_SAMPLED:
                        MySamplingFlags samplingFlags = tryParseSamplingFlags(value.charAt(pos - 1));
        ...
        return MyTraceContextOrSamplingFlags.create(new MyTraceContext(
                flags,
                traceIdHigh,
                traceId,
    ...
    @Nullable static MySamplingFlags tryParseSamplingFlags(char sampledChar) {
        switch (sampledChar) {
            case '1':
                return MySamplingFlags.SAMPLED;
            case '0':
                return MySamplingFlags.NOT_SAMPLED;
            case 'd':
                return MySamplingFlags.DEBUG;
            default:
